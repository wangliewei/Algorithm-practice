public class RestoreWays {
     ...
     * 思路：
     * 一、对应下面的ways1方法
     * 定义一个int f(arr,i,v,s),  含义：arr即所谓数组，把arr[i]变成v，
     * s有三种值0,1,2；分别表示v小于、等于、大于arr[i+1];  返回的是arr[0...i]在这种情况下
     * 满足条件的可能的结果有多少种
     *
     *
     *
    ...
    public static int ways1(int[] arr) {
        int N = arr.length;
        if (arr[N - 1] != 0) {
            return process1(arr, N - 1, arr[N - 1], 2); //注意这里的2，要好好理解
        } else {
            int ways = 0;
            for (int v = 1; v < 201; v++) {
